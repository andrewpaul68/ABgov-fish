...
If you are unable to authorize _JAVA_, you will need to save the FWIS files into a _csv_ format.


## Getting the FWIS data
Information on each watershed is extracted from the FWIS database. Contact Chad Sherburne (780.638.4143) for further details.
...
It may be difficult to get _Java_ to work properly on many computers. For example, on Macintosh computers,
the security settings are set high enough that _Java_ cannot be called from _R_ unless you are able
to make deep modifications to system files. 
...
If you are unable to get _Java_ to work on your computer, open the FWIS data file and save the 'F&W' sheet as a _csv_file.
...
### Checking that _Java_ is configured properly.
...
Select the _R_ code fragment
```{r echo=TRUE, eval=FALSE}
library(xlsx)
```
and run it. If the _xlsx_ package loads without error, then _Java_ has been configured properly. 

If the _xlsx_ package does NOT load properly, then comment out the line so it looks like:
```{r echo=TRUE, eval=FALSE}
#library(xlsx)  # comment out this line if Java does not work properly
```
...
If _Java_ is not working, you will need to save the 'F&W' sheet as a _csv_ file using Excel. Then specify the file 
name as:
```{r echo=TRUE, eval=FALSE}
...
#### Read in the data (_JAVA_ working)
Choose either this  code fragment if _Java_ is working or the next code fragement.
...
#### Read in the data (_JAVA_ NOT working)
...
# read the sheet from the workbook   
cpue <- read.FWIS.workbook.csv(    # notice the suffix on the function call
        csvfilename,   
        sheetName     =sheetName,   
        target.species=target.species)   
```
...
library(scales)
library(stringr)
#library(xlsx)  # This requires JAVA to work properly
...
can exclude data, for example, from angling surveys that are not amenable to the CPUE computations. Some of the
